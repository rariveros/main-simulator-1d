...
if __name__ == '__main__':
    ...
    # Initial Conditions
    for i in range(Nt):
        u_real[i, :] = envelope * np.cos(np.pi * k * x_grid + omega_01 * t_grid[i]) + delta_r * np.cos(np.pi * q * x_grid) * np.cos(omega_02 * t_grid[i])
        u_img[i, :] = envelope * np.cos(np.pi * k * x_grid + omega_01 * t_grid[i]) + delta_i * np.cos(np.pi * q * x_grid) * np.sin(omega_02 * t_grid[i])
